...
@section('admin-content')
    ...
    <div class="table-responsive"><table class="table table-hover">
        ...
        <tbody>@forelse($withdrawals as $withdrawal)
            <tr><td>{{ $withdrawal->created_at }}</td><td>{{ optional($withdrawal->wallet->user)->username }}</td><td>{{ strtoupper($withdrawal->coin) }}</td><td>{{ $withdrawal->amount_display }} {{ strtoupper($withdrawal->coin) }}</td><td class="text-break">{{ $withdrawal->destination_address }}</td><td><span class="badge badge-{{ $withdrawal->status === 'broadcast' ? 'success' : ($withdrawal->status === 'failed' ? 'danger' : 'warning') }}">{{ str_replace('_', ' ', $withdrawal->status) }}</span>@if($withdrawal->transaction_hash)<div class="small text-break mt-1">TX: {{ $withdrawal->transaction_hash }}</div>@endif @if($withdrawal->admin_note)<div class="small text-danger mt-1">{{ $withdrawal->admin_note }}</div>@endif</td><td>
